...
import { activateBoard, addBoard, getActiveInstanceId } from '../board/boardStore'
...
beforeEach(() => {
  ...
  addBoard(3) // MoonBoard Masters 2024 (any known board that isn't the session's)
  activateBoard('3')
})
...
describe('navigateToSessionBoard', () => {
  ...
  it('falls back to /boards when the board layout id is unknown, and does not touch active board', () => {
    ...
    expect(getActiveInstanceId()).toBe('3') // unchanged
