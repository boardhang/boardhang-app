...
- KTD12. **Migration split mirrors 0003/0004: storage first, then RPCs.** U1 = `0016_social_graph.sql` (columns, tables, helpers, trigger, RLS, backfill); U2 = `0017_social_rpcs.sql` (all SECURITY DEFINER RPCs + the projection core). Each migration is independently reviewable and test-first. Migration numbers assume no parallel branch claims 0016/0017 first — renumber on rebase if so (flagged in Risks).
...
### U1. Migration `0016_social_graph`: columns, tables, helpers, trigger, RLS, backfill
...
### U2. Migration `0017_social_rpcs`: follow/block/search/feed RPCs + projection core
...
- **Approach:** `feedStore` pages `get_follow_feed` (keyset on `first_sent_at, id`), **read-through cached** to a single last-fetch snapshot (KTD10) so re-opening offline shows stale content marked "offline — last updated X" with no actions and no new fetch. `FeedItem` renders actor + problem + **climb-date** relative time (display key ≠ sort key, KTD3); tapping opens the problem via the existing `?problem=<source_catalog_id>` drawer navigation (the KTD9 idiom from the queue plan). Read-only — no reaction affordances (R16). Enumerate all first-open states so the headline surface isn't a guess: **first-load** (no cache yet) shows a skeleton/spinner, not the empty prompt; **online-fetch-error with no cache** shows an error + retry affordance (distinct from the offline-stale path); **offline-with-cache** shows the stale snapshot marked "offline — last updated X"; **empty graph** (viewer follows no one) → routes to discovery (U4); **populated** renders the list.
...
- **OPEN DECISION — bulk logbook import floods the feed (from doc review, deferred to you).** A user who already has followers and then imports a large historical logbook (e.g. the MoonBoard import) writes many `sent=true` rows at once; each stamps `first_sent_at = now()`, so all of them land together at the top of every follower's feed and bury other friends' activity. `first_sent_at`'s arrival-sort is right for a single late upload (AE1) but has no absorber for a burst (R17 declines grouping/dedup in v1). This is **not yet resolved** — candidate fixes: cap per-actor items shown per feed page, collapse a same-arrival burst into one "Ana logged 40 sends" feed entry, or stamp bulk-imported rows from their climb `date` instead of `now()`. Decide before U5; until then the feed is exposed to this flood.
...
- **Abuse (follow-spam / scraping):** Mitigation: unique-edge, min-length + hard-limited search, blocking (KTD11). A per-user follow-velocity limiter is a named later addition, not built speculatively.
- **Migration numbering:** 0016/0017 assume no parallel branch claims them first (several worktrees are active). Renumber on rebase if a collision appears.
