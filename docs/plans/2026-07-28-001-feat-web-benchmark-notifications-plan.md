...
> **Amendment 2026-07-29 — iOS push descoped from the first push shipment.** Phase 3 now
> targets Chrome/Firefox/Edge only (Android + desktop). iOS installed PWAs continue to receive
> the Phase 1 in-app surfaces (banner + dot + `?newSince` deep link); app-closed push on iOS is
> follow-up work. This removes the U7 iOS declarative-push spike gate and simplifies KTD5 / U4 /
> U5 / U6. Adding iOS push later is a strict superset: the sender stays declarative-shape-ready,
> the SW handler stays the render path for non-Safari, and a future spike + payload extension is
> a small delta. See the "iOS push (deferred follow-up)" section under Scope Boundaries.

# New-Benchmark Notifications - Plan

## Goal Capsule

- **Objective:** Notify users when problems on a board they have added become benchmarks — an in-app banner for everyone, opt-in web push for signed-in users on Chrome/Firefox/Edge (Android + desktop) — aggregated per board and angle, deep-linking to a filtered catalog view of exactly the new problems. iOS installed PWAs receive the in-app surfaces only (app-closed push is deferred follow-up work — see Scope Boundaries).
- **Product authority:** The user (product owner). All product decisions in this doc were made explicitly in the planning dialogue.
- **Execution profile:** Six units in three phases (post-amendment; U7 removed). Phase 1 (U1–U2) is the server events foundation plus the in-app surfaces — most of the user value, none of the push risk. Phase 2 is the U3 service-worker migration (needed regardless of iOS to give Chrome/Android a place for a `push` handler). Phase 3 (U4–U6) is Chrome/Firefox/Edge push. `supabase/migrations/**` is safety-critical per [AGENTS.md](../../AGENTS.md): U1 is planned test-first, runs at maximum reasoning effort, and review is mandatory.
- **Stop conditions:** Stop and surface rather than guess if the RLS tests cannot express an assertion without weakening a policy, or if the service-worker port cannot reproduce any of the four documented Workbox behaviors byte-for-byte in effect.
...
- R6. A "New benchmark alerts" toggle in Settings requests browser permission on tap. Supported on Chrome/Firefox/Edge (Android + desktop). **On any iOS-like context — Safari, Bluefy, or an installed PWA — the toggle renders a plain "Not yet available on iOS" state instead**, with no permission prompt and no subscription attempt. The in-app banner + dot + `?newSince` deep link remain the iOS surface (Phase 1 already ships these).
...
- AE6. **Fresh install deep link (covers R5).** Given a push tapped on a device where the board isn't added (e.g. a freshly installed Android Chrome PWA), then the filtered view renders in preview mode with the existing "Add this board" banner.
- AE7. **iOS toggle state (covers R6, amendment).** Given a signed-in user on iOS (Safari, Bluefy, or an installed PWA) opens Settings, then the "New benchmark alerts" section renders "Not yet available on iOS" — no permission prompt is triggered, no subscription attempt is made, and the in-app surfaces continue to work.

### Scope Boundaries

- **iOS push (deferred follow-up, amendment 2026-07-29):** app-closed push notifications on installed iOS PWAs are out of scope for this shipment. iOS users get the in-app surfaces from Phase 1 (banner, dot, `?newSince` deep link). Adding iOS push later is intentionally a strict superset of what ships here: run the (previously-U7) declarative-push spike on a real iPhone; if it passes, extend the sender's payload with the declarative fields (`web_push: 8030`, `notification.title`, `notification.body`, `notification.navigate`, `Content-Type: application/notification+json`) — Chrome/Firefox/Edge ignore those fields and continue firing the SW `push` handler unchanged; flip the R6 Settings copy from "Not yet available on iOS" to the install-to-home-screen path; VAPID keys carry over (same keypair, no subscriber wipe). The bounded rework: sender payload, one Settings branch, one row in the docs.
...
- KTD4. **Service worker migrates to `injectManifest`** (own unit, own PR). The `importScripts` shortcut was evaluated and rejected: imported-script update propagation depends on `updateViaCache` defaults and browser-version behavior, and a `public/` file sits outside `tsc -b`/oxlint. The migration was trial-applied during research: byte-identical precache manifest, and the exact port of all four Workbox behaviors is in U3. Critical gotcha: `registerType: 'autoUpdate'` does nothing under injectManifest — `self.skipWaiting()` + `clientsClaim()` must be explicit in `sw.ts`. **Amendment note:** the SW rewrite is still required for Chrome/Firefox/Edge push (they need a `push` handler; `generateSW` has no place to put one), independently of the iOS decision.
- KTD5. **Push payload is standard imperative web push JSON** — `{ "title": "...", "body": "...", "data": { "url": "..." } }` — parsed in the SW's `push` handler and rendered via `registration.showNotification()`. Sender sets `TTL: 86400` (pywebpush defaults to 0 — undelivered-if-asleep), `Topic` per `(layout, angle)` (an undelivered older digest is replaced, making at-least-once benign), `Urgency: low`, and an explicit request timeout. Non-declarative — iOS is out of scope for this shipment. **Superset for a future iOS extension:** add `web_push: 8030` + `notification.title / body / navigate` + `Content-Type: application/notification+json` alongside the existing fields; Chrome/Firefox/Edge ignore these and keep firing the SW handler, iOS/macOS Safari renders declaratively. VAPID keys carry over unchanged.
...
flowchart TB
  ...
  subgraph server [Sender]
    ...
    NI --> DR
    PS --> DR
    DR -->|VAPID web push,<br/>Topic per slab| SVC[push services<br/>FCM / Mozilla]
  end
...
- U3 and U5 target Chrome/Firefox/Edge (Android + desktop). Bluefy (WKWebView) never receives push regardless of platform — its users get the in-app surfaces. iOS Safari + installed iOS PWAs also get in-app-only for this shipment; adding declarative push later is bounded rework per Scope Boundaries.
- The user-base Android/desktop/iOS split is unmeasured; if it turns out overwhelmingly iOS, watch Phase 1 banner engagement before committing to the Phase 3 push build at all (an option the Deferred/Open Questions section makes explicit).
...
### ~~U7. iOS declarative-push spike~~ — **Removed by 2026-07-29 amendment.** iOS push is deferred follow-up work (see Scope Boundaries → "iOS push"). If iOS push is scheduled later, restore the spike as a prerequisite; the recipe is preserved in git history at commits before this amendment.
...
- **Dependencies:** parallel to U1/U2; must land before U5/U6.
...
- **Dependencies:** U1. Independent of U2/U3 (a push can be proven against a hand-inserted subscription row before the client ships).
...
- **Goal:** Chrome/Firefox/Edge render pushes and taps land on the filtered view.
- **Requirements:** R7 (client half).
- **Dependencies:** U3, U4 (payload shape fixed).
- **Files:** `web/src/sw.ts`.
- **Approach:** `push`: parse `event.data?.json()` in try/catch (fallback title on malformed/absent payload — a received-but-unshown push costs quota), always `showNotification` inside `event.waitUntil` with `{ title, body, data: { url } }` from the payload, set `tag` to the Topic value for client-side collapse. `notificationclick`: `notification.close()`, then focus-or-open inside `waitUntil` — `clients.matchAll({ type: 'window', includeUncontrolled: true })`, navigate an existing client to `data.url` and focus it, else `clients.openWindow(data.url)`. Keep handlers dumb: no fetches, no IndexedDB.
...
- **Verification:** real Android Chrome + a desktop Chrome/Firefox: push arrives app-closed, tap opens the filtered view; app-open push still shows; malformed payload shows fallback rather than nothing. (iOS installed PWA testing is deferred — R6 hides the toggle there.)
...
- **Approach:**
  ...
  - Settings section per `CatalogCacheSection.tsx` + `PreviewToggleRow` idiom. **iOS branch (any iOS-like context — Safari, Bluefy, installed PWA — detected via `isIosLike()` from `lib/pwa.ts`) renders a "Not yet available on iOS" state (R6, amendment): no permission prompt, no subscription attempt, no capability probing. Everything else falls through to the Chrome/Firefox/Edge path.** Nudge: one dismissible strip for signed-in, capability-positive, not-opted-in users; `safeGetItem`/`safeSetItem` permanence. Nudge also suppressed under `isIosLike()`.
...
- All implementation units (U1–U6, with U7 removed by the 2026-07-29 amendment) landed through PRs per tier rules (U1 safety-critical with mandatory review), each with its docs updated in the same commit (`docs/catalog-data-pipeline.md` for U4, `docs/navigation-and-ui-flows.md` for U2/U6).
...
- **No go/no-go gate between the in-app phase and the push phase** (adversarial review, P1). Signed-in share × opt-in rate × Android/desktop share are all unmeasured; nothing in the plan names a Phase 1 result that would stop Phase 3. Suggested: pick an observed signal (banner views per event, distinct devices seeing a banner) and a floor before starting U4–U6. **Default if unanswered:** phases proceed in order.
