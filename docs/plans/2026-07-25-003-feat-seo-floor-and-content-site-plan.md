...
- **Tail:** PR per repo convention (PR-first, no direct push to `main`); deploys are manual CLI and are part of U5's verification, not of merging.
...
U1 (web floor) and U2 (site scaffold) are independent and can land in either order. U3/U4 build on U2. U5 (ops) needs U1–U4 built — the boardly deploy ships U1, and the domain move needs the landing page live — and carries the deployed verification for all of them. U6 (docs) closes out, same PR.
...
- **Approach:** ordered steps, each with its check. The `boardly` deploy runs **before** the Cloudflare toggle: until U1 is deployed, the origin still serves the SPA shell for `/robots.txt`, so the toggle's check could not pass in the reverse order.
  1. *(user)* Register `boardhang.com` — defensive only; redirect wiring deferred.
  2. *(CLI)* Deploy `boardly` from a clean tree per `web/CLAUDE.md`. Checks: `curl -I` shows `X-Robots-Tag: noindex`; `/sitemap.xml` returns XML; OG tags present in `curl` of `/`. (`/robots.txt` still carries the Cloudflare-injected block until step 3.)
...
- **Approach:** `docs/content-site.md` owns depth: two-origin architecture and why cross-host separation protects the PWA (SW `navigateFallback`), the canonical URL scheme (apex canonical now, reserved `/problems/<layout>/<slug>-<id>` shape), the deep-link mapping table (`/board/{layoutId}/catalog?problem={source_catalog_id}`, strip-at-default rule), the noindex posture, and the future kill-switch requirement. `docs/README.md` gets a "When to read what" row; `CONTEXT.md` gets a `site/` repo-map row and a "Where to read next" link — summaries only, no restating (deploy runbook stays in `site/CLAUDE.md`).
...
- U5 manual steps either completed (with checks passing) or explicitly handed to the user with the runbook — the code PR does not block on dashboard actions, but the plan is not "live" until step 2's robots check passes.
