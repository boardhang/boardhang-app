...
The PWA (`web/`) ports the registry and the membership model unchanged, on the same
`localStorage` keys so state stays traceable across apps:
...
- `web/src/board/boardInstance.ts` + `web/src/board/boardStore.ts` — added/active/per-board
  state, keyed by **instance** rather than layout id (below).

### Board instances (web only)

The web app adds a layer iOS does not have. A board the user holds is a **`BoardInstance`** —
an instance *of* a registry layout, not the layout itself — so one layout can back more than
one board at a time. That is what lets someone hold their own MoonBoard 2019 and also a shared
MoonBoard 2019 at their gym, each with its own angle, installed hold sets, and flip calibration.

`addedBoards` and `activeBoardId` therefore hold **instance ids**, and the per-instance keys are
`angle_<instanceId>` / `flipped_<instanceId>` / `activeHoldSets_<instanceId>`.

An instance id is **opaque and immutable**. Two shapes exist, for migration reasons only:

| Shape | Meaning |
| --- | --- |
| `"7"` | A local instance whose id is the bare layout id. Every instance predating this model has it, so existing keys keep working and the migration performs no writes. |
| `"S:<boardId>"` | An instance created by joining a shared board. |

The shape is **not** a shared-ness signal. An id is assigned once and never re-keyed: a local
instance that gets shared keeps its bare id, and a shared instance that stops being shared keeps
its `S:` id. Read shared-ness from the instance's `sharedBoard_<accountId>_<instanceId>` mirror
instead. Re-keying on either transition would break something concrete — copying three keys
mid-flow one way, and colliding with a sibling local instance of the same layout the other.

Two consequences of that follow, both easy to get wrong:

- An `S:` id with **no mirror** is a half-written adoption. It is dropped from the snapshot
  *without* writing, so a transient read failure cannot destroy state that becomes valid again
  once the mirror lands.
- A **demoted** instance also has an `S:` id and no mirror, so it pins its layout in
  `instanceLayout_<instanceId>`. That pin is the only thing distinguishing it from an orphan.

Mirrors are namespaced by the account that adopted them and cleared when a *different* account
signs in (`syncBoardsIdentity`, wired into `AuthProvider` beside the lists/sessions/beta
equivalents). Signing out deliberately keeps them — an adopted board has to survive sign-out and
the owner deleting their account. Purely local instances stay device-wide.
...
- Layout id (Int) is the real key on iOS; `Board`/`MoonBoardSetup` are wrappers around it. On web
  the per-board key is the **instance id**, and shared-ness is read from the mirror, never from
  the id's shape.
